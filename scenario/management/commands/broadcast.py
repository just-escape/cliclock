    ...
    def notify_from_slug(slug, notification, message):
        if notification == "reload":
            ...
            wsn.notify(
                slug,
                {
                    "type": MT.PUSH_MESSAGE,
                    "data": {"content": message, "level": "info"},
                },
            )
            ...
            wsn.notify(
                ...
                {
                    "type": MT.PUSH_MESSAGE,
                    "data": {"content": message, "level": "success"},
                },
            )
            ...
            wsn.notify(
                ...
                {
                    "type": MT.PUSH_MESSAGE,
                    "data": {"content": message, "level": "error"},
                },
            )
        else:
            print("Do nothing")
    ...
    def handle(self, *args, **options):
        ...
        if all_players:
            for p in sm.Player.objects.all():
                self.notify_from_slug(
                    p.slug, options["notification"], options["message"]
                )
